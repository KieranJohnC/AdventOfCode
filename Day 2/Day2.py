...
# Their password database seems to be a little corrupted: some of the passwords wouldn't have been allowed by the Official Toboggan Corporate Policy that was in 
...
with open("input.txt") as passwordFile:
  passwords = list(passwordFile.readlines())
  validPasswords = []

  for i in passwords:
    i = i.split(" ")
    minVal, maxVal = i[0].split("-")
    char = i[1].strip(":")
    string = i[2].strip("\n")
    totalOccurrence = Counter(string)

    if int(minVal) <= int(totalOccurrence[char]) <= int(maxVal):
      validPasswords.append(string)
...
with open("input.txt") as passwordFile:
  ...
  for i in passwords:
    i = i.split(" ")
    firstValue, lastValue = i[0].split("-")
    char = i[1].strip(":")
    string = i[2].strip("\n")
    firstValue = int(firstValue) - 1
    lastValue = int(lastValue) - 1
    
    if string[firstValue] == char and string[lastValue] == char:
      continue

    if string[firstValue] == char or string[lastValue] == char:
      validPasswords.append(string)
